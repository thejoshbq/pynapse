...
class ZScore:
    """
    Z-score entire trace using statistics from a baseline window.

    Parameters
    ----------
    window_ms : tuple[float, float], default (-3000, -500)
        Pre-stimulus window in ms (classic: -3 to -0.5 sec before trial start).
    frame_duration_ms : float
        Frame duration in milliseconds. Required.
    """

    def __init__(
        self,
        window_ms: Tuple[float, float] = (-3000.0, -500.0),
        frame_duration_ms: float = 33.3,  # ~30 Hz default
    ) -> None:
